    ...
    <div class="wrap view">
      <div class="card">
        <div class="card-header">
          <h1 class="card-header-title">{{newsContent.title==""?"快来发表你的第一条留言吧":newsContent.title}}</h1>
        </div>
        <div class="card-body">{{newsContent.title==""?"竟然是第一个进来的，快来体验神秘世界吧！":newsContent.body}}</div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      commentslength: 0,
      newsContent: {},
      userAvatar: "http://www-luluisme.gz01.bdysite.com/",
      start: 0,
      end: 10,
      content: '',
      aaa:"",
  ...
  methods: {
    ...
    getChatall(start, end) {
      ...
      this.axios
        .get("http://www-luluisme.gz01.bdysite.com/api/getchatAll.php?chatId=" + this.id)
        .then(res => {
          // reverse
    ...
    chatAddContent(){
       // let formData = this.content;
        ...
        this.axios.post('http://www-luluisme.gz01.bdysite.com/api/messageAdd.php',ly).then(res=>{
          // console.log(res)
          })
          .catch(res=>{
          // console.log(res)
